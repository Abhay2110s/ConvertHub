...
export default function CalculatorLayout({ title, description, eyebrow = "ConvertHub Tool", accent = "#8B5CF6", children }) {
  return (
    <>
      <DashboardHeader />
      <main className="relative min-h-screen w-full overflow-hidden bg-[#FFF8EC]">
      ...
      <div className="relative z-10 mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-10">
        ...
        <section className="py-9 sm:py-12">
          ...
          <motion.h1 initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:.08}} className="mt-6 text-4xl font-black leading-[0.95] sm:text-6xl lg:text-7xl" style={{fontFamily:"'Fredoka', sans-serif"}}>
            {title}<span style={{color:accent}}>.</span>
          </motion.h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-600 sm:text-lg">{description}</p>
        </section>
        {children}
      </div>
      </main>
